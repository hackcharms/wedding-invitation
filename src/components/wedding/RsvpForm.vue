...
const processRSVP = (): void => {
  ...
  alert(
    `Assalamu Alaikum ${formData.name}. Your family response has been recorded.`,
  );
};
</script>
...
<template>
  <section
    class="max-w-sm mx-auto w-full p-6 bg-luxury-card/90 backdrop-blur-md rounded-xl border border-luxury-gold/20 shadow-xl"
  >
    <div class="text-center mb-6">
      <span class="text-xs text-luxury-gold">💌</span>
      <h3
        class="font-serif text-xs tracking-[0.25em] uppercase mt-1 text-transparent bg-clip-text bg-gradient-to-b from-white to-luxury-cream"
      >
        Send Prayers &amp; RSVP
      </h3>
    ...
    <form @submit.prevent="processRSVP" class="space-y-4">
      ...
      <div
        v-if="formData.attendance === 'accept'"
        class="space-y-4 pt-2 border-t border-white/[0.04] animate-fade-in"
      >
        <BaseSelect
      ...
      <div class="w-full space-y-1 font-sans text-xs">
        <label class="block text-gray-400 tracking-wider font-medium"
          >Warm Wishes / Custom Prayers</label
        >
        <textarea
      ...
      <BaseButton type="submit" variant="primary"> Submit Response </BaseButton>
    </form>
    <p class="pt-8 text-[9px] font-sans tracking-[0.2em] text-white uppercase">
